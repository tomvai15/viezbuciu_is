...
export default function AddFood() {
  ...
  const [foodOrders, setFoodOrders]=React.useState([]);
    ...
    useEffect(() => {
      clientServices.getFoodOrders(id).then((res)=>{
        const foodOrders = res.data.data;
        setFoodOrders(foodOrders.map(foodOrder=>createData(foodOrder.pristatymo_data, foodOrder.pavadinimas,  foodOrder.kiekis, foodOrder.name, foodOrder.kaina)))
      },
      error => {
        const resMessage =
          (error.response &&
    ...
    useEffect(() => {
      clientServices.getMeniuItems().then((res)=>{
        const items = res.data.data;
        setItems(items.map(item=>createItems(item.id_Meniu_irasas, item.pavadinimas)))
      },
      error => {
        const resMessage =
          (error.response &&
    ...
    useEffect(() => {
      clientServices.getFoodOrderTypes().then((res)=>{
        const types = res.data.data;
        setTypes(types.map(type=>createItems(type.id_Pristatymo_tipas, type.name)))
      },
      error => {
        const resMessage =
          (error.response &&
  ...
  const handleSubmit = ( event) => {
    const data = new FormData(event.currentTarget);
      if( date== null || data.get('item') == null || data.get('amount')==null || data.get('type')==null ){
        ...
        clientServices.addFoodOrder(foodOrderData).then((res)=>{
        },
        error => {
          const resMessage =
            (error.response &&
